...
def main(gpu, ngpus_per_node, opt):
    ...
    if opt.start_from is not None and os.path.isfile(os.path.join(opt.start_from, 'model.pth')):
        model.load_state_dict(torch.load(os.path.join(opt.start_from, 'model.pth'), map_location=lambda storage, loc: storage), strict=False)
    ...
    ##########################
    if opt.noamopt:
        assert opt.caption_model in ['transformer', 'transformer_c3cap', 'bert', 'm2transformer'], 'noamopt can only work with transformer'
    ...
    if opt.start_from is not None and os.path.isfile(os.path.join(opt.start_from,"optimizer.pth")):
        optimizer.load_state_dict(torch.load(os.path.join(opt.start_from, 'optimizer.pth')), map_location=lambda storage, loc: storage)
