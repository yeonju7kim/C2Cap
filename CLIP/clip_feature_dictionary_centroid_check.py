...
class CLIPClusterConstructor():
    def __init__(self, clip_feature_hdf, train_folder, annotation_path, clip_cluster_pkl, clip_collections_pkl, k, aug_num):
        ...
        if os.path.exists(self.clip_cluster_pkl) == False:
            ...
            cluster_ids, cluster_centers = self._cluster_kmean(image_features, k)
            self.save_clip_cluster(image_features, cluster_ids, cluster_centers)
    ...
    def _average_clip_cluster(self, image_features, cluster_ids):
        ...
        for k, v in features_by_cluster.items():
            averaged_features_in_cluster[k] = np.sum(np.array(v), axis=0) / len(v) #np.array(v).mean()
    ...
    def save_clip_cluster(self, image_features, cluster_ids, cluster_centers):
